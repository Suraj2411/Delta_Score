...
def get_bert_embedding(all_sens, model, tokenizer, idf_dict, device="cuda:0"):
...
def div_matrices(hyp_embed, ref_embed):
    ref_embed = ref_embed.to(torch.float64)
    hyp_embed = hyp_embed.to(torch.float64)

    ref_embed = torch.nn.functional.softmax(ref_embed, dim=2)
    hyp_embed = torch.nn.functional.softmax(hyp_embed, dim=2)

    # kl_div_mat = torch.zeros(hyp_embed.size(0), hyp_embed.size(1), ref_embed.size(1))
    js_div_mat = torch.zeros(hyp_embed.size(0), hyp_embed.size(1), ref_embed.size(1))

    for i in range(hyp_embed.size(0)):
        for j in range(hyp_embed.size(1)):
            # kl_div_mat[i, j] = kl_divergence(hyp_embed[i, j].unsqueeze(0), ref_embed[i]).squeeze(0)
            js_div_mat[i, j] = js_divergence(hyp_embed[i, j].unsqueeze(0), ref_embed[i]).squeeze(0)

    return js_div_mat
    # return kl_div_mat, js_div_mat

def divergence_score(ref_embedding, ref_masks, ref_idf, hyp_embedding, hyp_masks, hyp_idf):

    ref_embedding.div_(torch.norm(ref_embedding, dim=-1).unsqueeze(-1))
    hyp_embedding.div_(torch.norm(hyp_embedding, dim=-1).unsqueeze(-1))

    js_div = div_matrices(hyp_embedding,ref_embedding)
    masks = torch.bmm(hyp_masks.unsqueeze(2).float(), ref_masks.unsqueeze(1).float())

    masks = masks.float().to(js_div.device)

    js_div = 1 - js_div
    js_div = js_div * masks
    js_div = 1 - js_div

    word_precision = js_div.min(dim=2)[0]
    word_recall = js_div.min(dim=1)[0]
...
def score_calc(model, refs, hyps, tokenizer, idf_dict, verbose=False, batch_size=64, device="cuda:0"):
    preds = []
    ...
    for batch_start in iter_range:
        ...
        embs, masks, padded_idf = get_bert_embedding( sen_batch, model, tokenizer, idf_dict, device=device)
        embs = embs.cpu()
        masks = masks.cpu()
    ...
    if verbose:
        print("Computing Divergence Score.")
        iter_range = tqdm(iter_range)
    ...
    with torch.no_grad():
        for batch_start in iter_range:
            ...
            P, R, F1 = divergence_score(*ref_stats, *hyp_stats)
